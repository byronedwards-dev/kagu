...
export function imgName(i) {
  return PAGE_FORMATS[i]?.name || `Image ${i + 1}`;
}
export function pageNum(i) {
  return PAGE_FORMATS[i]?.print || null;
}
export function imgFmt(i) {
  return PAGE_FORMATS[i]?.format || "square";
}
...
export const AGE_RANGES = ["2–3 years", "4–6 years", "5–7 years", "7–9 years"];
...
export const COMPANION_ROLES = ["Comic relief", "Supportive sidekick", "Learning alongside", "Contrast character"];
export const STRUCTURES = ["Story with arc", "Structural pattern", "Hybrid"];
export const DIRECTIONS = ["Learning something new", "Preparing for an event", "Overcoming a challenge", "Exploration / discovery", "Emotional growth", "Other"];
...
export const TEXT_DENSITIES = ["Very light (8+ image-only)", "Standard (5-7 image-only)", "Text-heavy (3-4 image-only)"];
