...
const LIGHT = {
  ...
  textDim: "#7A7488",
  accent: "#7B6CE0",
  accentBg: "rgba(123,108,224,0.08)",
  green: "#22863A",
  amber: "#B86E00",
  red: "#D73A49",
};
...
export const T = new Proxy(_current, {
  get(_, key) { return _current[key]; },
});
...
export function setThemeMode(mode) {
  ...
  Object.assign(_current, next);
  if (typeof document !== "undefined") {
    document.body.style.background = _current.bg;
  }
  _listeners.forEach(fn => fn(mode));
}
export function onThemeChange(fn) { _listeners.add(fn); return () => _listeners.delete(fn); }
export function initTheme() {
  if (typeof window === "undefined") return;
  const saved = localStorage.getItem("kagu-theme");
