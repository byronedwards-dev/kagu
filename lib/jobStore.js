...
export async function addPageResult(jobId, pageIdx, variants) {
  ...
  // Write to Blob (separate file per page — no overwrite needed)
  if (useBlob()) {
    try {
      await writeBlobOnce(pagePath(jobId, pageIdx), pageData);
      console.log(`[jobStore] Wrote blob page-${pageIdx} for job ${jobId}`);
    } catch (err) {
      console.error(`[jobStore] Blob page-${pageIdx} write failed: ${err.message}`);
    }
  }
}
...
export async function getJob(jobId) {
  if (useBlob()) {
    try {
      ...
      return {
        status: completedPages >= meta.totalPages ? "done" : meta.status,
