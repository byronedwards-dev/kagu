...
//   kagu-jobs/{jobId}/result-{ts}.json   — created once per callback by n8n-callback
//
// Each result file contains { pageIdx, variants, model, ts }.
// Multiple callbacks for the same page (from different models) create separate files.
// Reading a job: list all blobs with prefix, merge results by pageIdx.
...
// Store results for a single page (called by n8n-callback, one call per page per model)
// Uses a unique filename per callback so multi-model results don't overwrite each other.
export async function addPageResult(jobId, pageIdx, variants) {
  const ts = Date.now();
  const suffix = `${pageIdx}-${ts}`;
  const pageData = { pageIdx, variants, ts };

  // Write to filesystem
  const jobDir = join(JOBS_DIR, jobId);
  writeFsFile(join(jobDir, `result-${suffix}.json`), pageData);

  // Write to Blob (unique file per callback)
  if (useBlob()) {
    try {
      const path = `kagu-jobs/${jobId}/result-${suffix}.json`;
      await writeBlobOnce(path, pageData);
      console.log(`[jobStore] Wrote blob ${path} (${variants.length} variants)`);
    } catch (err) {
      console.error(`[jobStore] Blob result-${suffix} write FAILED for job ${jobId}: ${err.message}`);
    }
  }
}
...
export async function getJob(jobId) {
  if (useBlob()) {
    try {
      ...
      // Find result blobs (result-*.json)
      const resultBlobs = blobs.filter(b => b.pathname.match(/\/result-\d+-\d+\.json$/));
      const results = {};

      for (const rb of resultBlobs) {
        const pageData = await readBlob(rb.url);
        if (pageData && pageData.pageIdx !== undefined) {
          if (!results[pageData.pageIdx]) {
            results[pageData.pageIdx] = { variants: [] };
          }
          // Merge variants from multiple callbacks (multi-model)
          results[pageData.pageIdx].variants.push(...(pageData.variants || []));
        }
      }

      const completedPages = resultBlobs.length; // count callbacks, not unique pages
...
function getJobFromFs(jobId) {
  ...
  try {
    if (existsSync(jobDir)) {
      for (const file of readdirSync(jobDir)) {
        const match = file.match(/^result-\d+-\d+\.json$/);
        if (match) {
  ...
  const completedPages = resultCount;
  const status = jobError ? "error" : completedPages >= meta.totalPages ? "done" : meta.status;
  return {
    status,
    created: meta.created,
