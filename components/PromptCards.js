...
export default function PromptCards({ prompts, loading, lidx, onAI, promptsStale, onGenPrompts, onGoImages, bannedWords, chars }) {
  ...
  return <div>
    ...
    <div style={{ display: "grid", gap: 6 }}>
      {prompts.map((p, i) => {
        ...
        const hasChars = chars && (p.prompt || "").length > 200; // rough check — full prompts are long
    ...
    {!loading && prompts.length > 0 && <div style={{ marginTop: 16, display: "flex", gap: 8, flexWrap: "wrap" }}>
      ...
      {promptsStale && <Btn ghost danger onClick={onGenPrompts}>↻ Regenerate (text changed)</Btn>}
    </div>}
  </div>;
}
